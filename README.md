...
## Prerequisites
- Install Zola (macOS example):
  ```bash
  brew install zola
  ```
- Other platforms: see the [Zola installation docs](https://www.getzola.org/documentation/getting-started/installation/).
...
1. Start the dev server:
   ...
   zola serve
   ```
2. Open the printed local URL (usually `http://127.0.0.1:1111`).
...
- Push to `main`. The GitHub Actions workflow (`.github/workflows/deploy.yml`) builds the site and publishes the `public/` directory to GitHub Pages.
...
- `.github/workflows/deploy.yml` – CI workflow to build and deploy with Zola.
